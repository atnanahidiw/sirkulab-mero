...
/// Emits a phase update to [onProgress] (keyword used for phase detection) and,
/// when [onTrace] is provided, a child-friendly narration line to the trace box.
/// [traceMessage] decouples the displayed narrative from the phase keyword.
void _emitProgress(
...
class FlutterGemmaModelRuntime implements ModelRuntime {
  ...
  Future<String> _generateWithToolCalling(
    ...
    required double topP,
    required int seed,
    required AnalysisStoryFormatter formatter,
    void Function(String phase, double progress)? onProgress,
    void Function(String token)? onToken,
    void Function(String trace)? onTrace,
  }) async {
    ...
    final chat = await model.createChat(
      ...
      isThinking: enableThinking, // Qwen3 thinking mode → better multi-step planning
    ...
    try {
      ...
      if (imageBytes != null) {
        _emitProgress(onProgress, 'Sending image...', 0.30,
      ...
      _emitProgress(onProgress, 'Sending prompt...', 0.35,
      ...
      while (true) {
        ...
        onTrace?.call(formatter.toolCall(toolName, args, pass));

        final result = await matchedSpec.execute(args);
        debugPrint('[Pass $pass] App-side tool result ($toolName): $result');
        onTrace?.call(formatter.toolResult(toolName, result, pass));
