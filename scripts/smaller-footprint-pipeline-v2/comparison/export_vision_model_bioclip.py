...
Produces the bundled assets consumed by `lib/services/vision_runtime.dart`:
  ...
  assets/models/image_encoder_bioclip.onnx        # image encoder
  assets/models/attribute_embeddings_bioclip.json # per-attribute label text embeddings
...
Model — CLIP-style image/text encoder (BioCLIP, ViT-B/16)
-------------------------------
BioCLIP (ViT-B/16) is loaded through open_clip from
`hf-hub:imageomics/bioclip`. The model already exposes a standard
...
USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/export_vision_model_bioclip.py   # downloads HF weights itself
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/validate_vision_model_bioclip.py # validate the produced assets
...
SUFFIX = "bioclip"
INPUT_SIZE = 224  # bioclip native size (import fallback); runtime resolves enc.input_size dynamically

# Human-readable label used in logs / summaries.
DISPLAY_NAME = "BioCLIP (ViT-B/16)"

# open_clip model id.
HF_MODEL = "hf-hub:imageomics/bioclip"
...
# ──────────────────────────────────────────────────────────────────────────
def load_bioclip(hf_model: str = HF_MODEL) -> VisionEncoder:
    """Load BioCLIP (ViT-B/16) via open_clip."""
    import torch
    import open_clip
