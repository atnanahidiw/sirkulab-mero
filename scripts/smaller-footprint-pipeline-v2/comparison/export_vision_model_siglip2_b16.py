...
Produces the bundled assets consumed by `lib/services/vision_runtime.dart`:
  ...
  assets/models/image_encoder_siglip2_b16.onnx        # image encoder
  assets/models/attribute_embeddings_siglip2_b16.json # per-attribute label text embeddings
...
Model — CLIP-style image/text encoder (SigLIP 2, ViT-B/16)
-------------------------------
SigLIP 2 (ViT-B/16) is loaded through open_clip from
`ViT-B-16-SigLIP2` with pretrained weights `webli`. The model already exposes a standard
...
USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/export_vision_model_siglip2_b16.py   # downloads weights itself
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/validate_vision_model_siglip2_b16.py # validate the produced assets
...
SUFFIX = "siglip2_b16"
INPUT_SIZE = 224  # siglip2_b16 native size (import fallback); runtime resolves enc.input_size dynamically

# Human-readable label used in logs / summaries.
DISPLAY_NAME = "SigLIP 2 (ViT-B/16)"

# open_clip model id and pretrained tag.
HF_MODEL = "ViT-B-16-SigLIP2"
...
# ──────────────────────────────────────────────────────────────────────────
def load_siglip2_b16(hf_model: str = HF_MODEL) -> VisionEncoder:
    """Load SigLIP 2 (ViT-B/16) via open_clip."""
    import torch
    import open_clip
...
def main():
    ...
    enc = load_siglip2_b16(args.hf_model)
