#!/usr/bin/env python3
"""Evaluate MobileCLIP2 (S2) zero-shot `visual_group` accuracy across prompt templates.
...
Outputs:
  - per-image JSONL → `<data-repo>/data/processed/vision_eval_visual_group_mobileclip2_s2.jsonl`
  - a note          → `<data-repo>/data/processed/README_vision_visual_group_mobileclip2_s2.md`
  - per-template accuracy printed to stdout.

USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/eval_vision_mobileclip2_s2.py
...
from export_vision_model_mobileclip2_s2 import DISPLAY_NAME, HF_MODEL, SUFFIX, INPUT_SIZE, load_mobileclip2_s2  # noqa: E402
