#!/usr/bin/env python3
"""Measure species-level image retrieval for the image-only DINOv2-small backbone.

DINOv2-small has **no text encoder**, but species retrieval is image↔image, so
the same prototype evaluation applies — this is the pure image-embedding baseline
(DINOv2's strength). Builds one ViT-S/14 centroid prototype per species from the
labeled images in the sibling `sirkulab-mero-data` repo, then evaluates:
...
Nothing is written to the app; this is a measurement for the model-compare matrix.

USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/eval_species_retrieval_dinov2_small.py
...
from export_vision_model_dinov2_small import DISPLAY_NAME, IMAGE_SIZE, MODEL_ID, SUFFIX
...
def load_dinov2_small(model_id: str = MODEL_ID):
    """Return (processor, model). Image-only — same head the exporter uses
    (DINOv2 pooler/CLS token, L2-normalized)."""
...
def main():
    ...
    print(f"Loading {DISPLAY_NAME}: {args.model_id}")
    enc = load_dinov2_small(args.model_id)
    ...
    for fold in folds:
        ...
        for sp in fold:  # OOD: unknown species
            kmat = np.stack([full_sp[t] for t in known])
            for s in by_sp[sp]:
...
retrieval (image-only DINOv2-small) and open-set rejection (the OOD router).
