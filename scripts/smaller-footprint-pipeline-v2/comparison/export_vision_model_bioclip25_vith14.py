...
Produces the bundled assets consumed by `lib/services/vision_runtime.dart`:
  ...
  assets/models/image_encoder_bioclip25_vith14.onnx        # image encoder
  assets/models/attribute_embeddings_bioclip25_vith14.json # per-attribute label text embeddings
...
Model — CLIP-style image/text encoder (BioCLIP 2.5, ViT-H/14)
-------------------------------
BioCLIP 2.5 (ViT-H/14) is loaded through open_clip from
`hf-hub:imageomics/bioclip-2.5-vith14`. The model already exposes a standard
...
USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/export_vision_model_bioclip25_vith14.py   # downloads HF weights itself
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/validate_vision_model_bioclip25_vith14.py # validate the produced assets
...
SUFFIX = "bioclip25_vith14"
INPUT_SIZE = 224  # bioclip25_vith14 native size (import fallback); runtime resolves enc.input_size dynamically

# Human-readable label used in logs / summaries.
DISPLAY_NAME = "BioCLIP 2.5 (ViT-H/14)"

# open_clip model id.
HF_MODEL = "hf-hub:imageomics/bioclip-2.5-vith14"
...
# ──────────────────────────────────────────────────────────────────────────
def load_bioclip25_vith14(hf_model: str = HF_MODEL) -> VisionEncoder:
    """Load BioCLIP 2.5 (ViT-H/14) via open_clip."""
    import torch
    import open_clip
