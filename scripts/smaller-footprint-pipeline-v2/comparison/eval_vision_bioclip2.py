#!/usr/bin/env python3
"""Evaluate BioCLIP 2 (ViT-L/14) zero-shot `visual_group` accuracy across prompt templates.
...
Outputs:
  - per-image JSONL → `<data-repo>/data/processed/vision_eval_visual_group_bioclip2.jsonl`
  - a note          → `<data-repo>/data/processed/README_vision_visual_group_bioclip2.md`
  - per-template accuracy printed to stdout.

USAGE
-----
  uv run --python .venv-export/bin/python scripts/smaller-footprint-pipeline-v2/comparison/eval_vision_bioclip2.py
...
from export_vision_model_bioclip2 import DISPLAY_NAME, HF_MODEL, SUFFIX, INPUT_SIZE, load_bioclip2  # noqa: E402
