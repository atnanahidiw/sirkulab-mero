...
Why this exists:
- `visual_group` is the weakest attribute in the current zero-shot pipeline.
- Frozen DINO-style features often work better with prototype / nearest-neighbor
  matching than with text-label matching.
- A prototype is the mean-like representative embedding for a group of example
  images.

Output:
  assets/models/visual_group_prototypes.pb

The script also writes an eval report to:
  <data-repo>/data/processed/vision_eval_visual_group_prototypes.jsonl
  <data-repo>/data/processed/README.md
...
from export_vision_model import INPUT_SIZE, load_talk2dino


HERE = Path(__file__).resolve()
APP_REPO = HERE.parents[2]
WORKDIR = HERE.parents[3]
...
def write_eval_report(out_dir: Path, args, n_images: int, metrics: dict, selected_strategy: str, proto_acc: float, text_acc: float, candidate_scores: dict, loo_acc: float):
    out_jsonl = out_dir / "vision_eval_visual_group_prototypes.jsonl"
...
Generated by `scripts/build_prototypes.py` in the **sirkulab-mero** app repo.
Re-run the script to regenerate.

## `vision_eval_visual_group_prototypes.jsonl`
...
def main() -> int:
    ...
    ap.add_argument("--output", default=str(APP_REPO / "assets/models/visual_group_prototypes.pb"))
    ap.add_argument("--text-embeddings", default=str(APP_REPO / "assets/models/attribute_embeddings_talk2dino.json"))
