...
Builds one DINO centroid prototype per species from the labeled images in the
...
USAGE
-----
  uv run --python .venv-export/bin/python scripts/eval_species_retrieval.py
...
Generated by `scripts/eval_species_retrieval.py`. Measures image↔prototype species
...
"""
    (out_dir / "README_species_retrieval.md").write_text(note)
    with (out_dir / "vision_eval_species_retrieval.jsonl").open("w") as f:
        for r in rows:
            f.write(json.dumps(r) + "\n")
    print(f"\nWrote {out_dir / 'README_species_retrieval.md'} and the per-image jsonl")
    return 0
