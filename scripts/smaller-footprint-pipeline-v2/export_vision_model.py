...
Produces the bundled assets consumed by `lib/services/vision_runtime.dart`:
  ...
  assets/models/image_encoder_talk2dino.onnx        # int8 image encoder
  assets/models/attribute_embeddings_talk2dino.json # per-attribute label text embeddings
...
USAGE
-----
  uv run --python .venv-export/bin/python scripts/export_vision_model.py            # downloads HF weights itself
  uv run --python .venv-export/bin/python scripts/validate_vision_model.py          # validate the produced assets
...
def export_onnx(enc: VisionEncoder, out_dir: str, quant: str) -> str:
    ...
    fp32 = os.path.join(out_dir, "image_encoder_talk2dino.fp32.onnx")
    final = os.path.join(out_dir, "image_encoder_talk2dino.onnx")
...
def export_text_onnx(enc: VisionEncoder, out_dir: str, quant: str) -> str:
    ...
    fp32 = os.path.join(out_dir, "text_encoder_talk2dino.fp32.onnx")
    final = os.path.join(out_dir, "text_encoder_talk2dino.onnx")
...
def dump_tokenizer(enc: VisionEncoder, out_dir: str) -> None:
    ...
    tk = enc.tokenizer
    vocab_path = os.path.join(out_dir, "clip_vocab_talk2dino.json")
    ...
    merges_path = os.path.join(out_dir, "clip_merges_talk2dino.txt")
...
def export_embeddings(enc: VisionEncoder, vocab: dict, out_dir: str) -> str:
    ...
    path = os.path.join(out_dir, "attribute_embeddings_talk2dino.json")
...
def main():
    ...
    print("[1/4] loading Talk2DINO (DINOv2 + text, CLS-saliency-pool) …")
