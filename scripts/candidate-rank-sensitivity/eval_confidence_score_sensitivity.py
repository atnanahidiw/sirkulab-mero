#!/usr/bin/env python3
"""Evaluate confidence-score sensitivity with fixed images and candidate order."""
...
from _common import (
    ...
    candidate_confidence_value,
    candidate_scientific_name,
    format_candidates_with_confidence,
    load_jsonl,
...
PROMPT_TEMPLATE = """You are evaluating confidence-score sensitivity in species identification.

Task:
- Inspect the image carefully.
- Choose the single best species from the candidate list.
- Candidate identities and order are fixed.
- The confidence scores shown with each candidate are part of the input and should be considered.
...
def prompt_candidates(candidates: list[dict]) -> list[dict]:
    return [dict(c) for c in candidates]


def permute_confidence_values(candidates: list[dict], seed: int, example_id: str, trial_id: str) -> list[dict]:
    copied = [dict(c) for c in candidates]
    confidences = [candidate_confidence_value(c) for c in copied]
    rng = random.Random(stable_int_seed(seed, example_id, trial_id, "confidence"))
    rng.shuffle(confidences)
    for cand, confidence in zip(copied, confidences):
        cand["confidence"] = int(round(confidence))
    return copied


def run_trial(engine, sampler, image_path: str, candidates: list[dict], data_repo: str) -> str:
    prompt = PROMPT_TEMPLATE.format(candidates=format_candidates_with_confidence(candidates))
...
def is_correct(prediction: dict, ground_truth_species: str, ground_truth_common_name: str) -> bool:
    ...
    return any(candidate and candidate in {gt_species, gt_common} for candidate in (pred_species, pred_common, pred_answer))


def highest_confidence_rank(candidates: list[dict]) -> int | None:
    if not candidates:
        return None
    best_idx = 1
    best_conf = candidate_confidence_value(candidates[0])
    for idx, cand in enumerate(candidates[1:], 2):
        conf = candidate_confidence_value(cand)
        if conf > best_conf:
            best_idx = idx
            best_conf = conf
    return best_idx
...
def expected_rows_per_example(trials: int) -> int:
    return 1 + trials
...
def main() -> int:
    ...
    parser.add_argument("--trials", type=int, default=DEFAULT_TRIALS, help="Number of confidence-permutation trials per example")
    parser.add_argument("--seed", type=int, default=42, help="Deterministic shuffle seed")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_DIR / "confidence_score_results.jsonl"), help="Output JSONL")
    ...
    expected_rows = expected_rows_per_example(args.trials)
    existing_counts = {}
    ...
    for index, example in enumerate(examples, 1):
        ...
        original_candidates = prompt_candidates(example.get("original_candidates") or [])
        ...
        original_row = {
            ...
            "trial_id": "original",
            "confidence_assignment": "original",
            "candidate_order": original_candidates,
            "candidate_identity_order": [candidate_scientific_name(c) for c in original_candidates],
            "displayed_confidence_order": [int(round(candidate_confidence_value(c))) for c in original_candidates],
            "highest_confidence_rank": highest_confidence_rank(original_candidates),
        ...
        for trial_idx in range(1, args.trials + 1):
            trial_id = f"permute-{trial_idx:02d}"
            candidates = permute_confidence_values(original_candidates, args.seed, example["example_id"], trial_id)
            ...
            row = {
                "example_id": example["example_id"],
                "trial_id": trial_id,
                "confidence_assignment": "permuted",
                "candidate_order": candidates,
                "candidate_identity_order": [candidate_scientific_name(c) for c in candidates],
                "displayed_confidence_order": [int(round(candidate_confidence_value(c))) for c in candidates],
                "highest_confidence_rank": highest_confidence_rank(candidates),
