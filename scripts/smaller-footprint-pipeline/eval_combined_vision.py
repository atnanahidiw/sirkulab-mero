...
Outputs:
  - `<data-repo>/data/processed/vision_eval_combined_visual.jsonl`
  - `<data-repo>/data/processed/README_combined_vision.md`
...
def main() -> int:
    ...
    ap.add_argument(
        "--embeddings",
        default=str(APP_REPO / "assets/models/attribute_embeddings_talk2dino.json"),
    )
    ap.add_argument(
        "--prototypes",
        default=str(APP_REPO / "assets/models/visual_group_prototypes.pb"),
    )
    ...
    out_jsonl = out_dir / "vision_eval_combined_visual.jsonl"
...
Generated by `scripts/eval_combined_vision.py` in the **sirkulab-mero** app repo.
Re-run the script to regenerate.

## `vision_eval_combined_visual.jsonl`
...
"""
    (out_dir / "README_combined_vision.md").write_text(note)
    print(f"\nWrote {out_jsonl}")
    print(f"Wrote {out_dir / 'README_combined_vision.md'}")
    db_conn.close()
    return 0
