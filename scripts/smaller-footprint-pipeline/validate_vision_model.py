#!/usr/bin/env python3
"""Validate the vision assets produced by `export_vision_model.py`.

Companion to the *create* program (`scripts/export_vision_model.py`). It checks
every shipped asset end-to-end, the same way `lib/services/vision_runtime.dart`
and `lib/services/clip_tokenizer.dart` use them on-device:

  1. assets present + ONNX sessions load + output shapes/finiteness
  2. CLIP tokenizer  — a mirror of the Dart byte-level BPE vs real clip.tokenize
                       (MUST be exact, or the text encoder gets garbage IDs)
  3. image encoder   — torch↔ONNX, and zero-shot attribute labels on real photos
                       (int8 top-1 labels must match fp32 — the metric that matters;
                        raw cosine diverges because saliency softmax amplifies int8)
  4. text encoder    — torch↔ONNX parity + check_visual_evidence claim scoring
                       (relative: real traits must outscore a wrong control claim)

USAGE
-----
  .venv-export/bin/python scripts/validate_vision_model.py
  #   --models-dir assets/models   --hf-model lorebianchi98/Talk2DINO-ViTB
...
tokenizer mismatch, low text-encoder parity, label disagreement). Semantic
...
def main() -> int:
    ...
    if checks.failures:
        print("\nMissing assets — run scripts/export_vision_model.py first.")
        return 1

    # ── 1. tokenizer parity (HARD: must be exact) ──
    print("\n[1/4] CLIP tokenizer ↔ clip.tokenize (Dart-mirror) …")
    import clip
    tok = DartTokenizerMirror(md)
    n_exact = 0
    for s in TOKENIZER_TESTS:
        mine, ref = tok.tokenize(s), clip.tokenize([s])[0].tolist()
        if mine == ref:
            n_exact += 1
        else:
            print(f"      mismatch {s!r}: mine={mine[:10]} clip={ref[:10]}")
    checks.hard(n_exact == len(TOKENIZER_TESTS), "tokenizer exact match",
                f"{n_exact}/{len(TOKENIZER_TESTS)}")

    # ── load model + sessions ──
    print("\n   loading Talk2DINO + ONNX sessions …")
    ...
    def torch_image(x):
        ...
        pooled = (w[:, None] * patch).sum(0)
        return l2(pooled.numpy())

    def onnx_image(x):
        return l2(isess.run(None, {"pixel_values": x})[0])[0]

    def onnx_text(s):
        ids = np.array([tok.tokenize(s)], dtype=np.int32)
        return l2(tsess.run(None, {"token_ids": ids})[0])[0]

    @torch.no_grad()
    def torch_text(s):
        return l2(model.encode_text([s]).cpu().numpy())[0]
    ...
    checks.hard(out_dim == 768, "image embedding dim == 768", str(out_dim))
    for name, path in images.items():
        x = preprocess(path)
        oe, te = onnx_image(x), torch_image(x)
