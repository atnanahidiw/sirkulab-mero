...
**Status:** ✅ done · **Owns:** the canonical `scripts/{export_vision_model,validate_vision_model,build_prototypes,eval_vision,eval_combined_vision,eval_species_retrieval}.py` flow plus the model-specific copies under `scripts/smaller-footprint-pipeline-v2/`
...
- Restored the model-specific script copies for the other backbones so they stay
  separate entrypoints, but grouped them in `scripts/smaller-footprint-pipeline-v2/`.
- Kept the comparison scripts explicit and self-contained under
  `scripts/smaller-footprint-pipeline-v2/comparison/` instead of wrapping them
...
- The other backbone-specific suffixed scripts now live in the
  `scripts/smaller-footprint-pipeline-v2/` subtree.
