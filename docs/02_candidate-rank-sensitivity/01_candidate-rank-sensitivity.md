...
Each frozen example preserves the same image, the same candidate identities, and the same decoding setup. The only variable we changed during evaluation was the candidate order. For every example, we ran one original-order pass, one shuffled-order pass, and one reverse-order pass. Confidence scores were removed before the prompt was shown, so the model saw the candidate names without an extra confidence cue that could influence selection. The evaluator compared the model’s answer after each perturbation against the original-order answer, and it also checked accuracy against the hidden ground-truth species when that label was available.
